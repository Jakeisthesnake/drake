...
class PdController : public drake::systems::VectorSystem<double> {
 public:
  PdController() : VectorSystem<double>(2 /* input size */, 1 /* output size */, true),
...
std::pair<drake::trajectories::PiecewisePolynomial<double>, std::vector<double>> MakeAngularTrajectory(
    double i_a, // initial angle
    double t_a, // travel angle
    double total_time,
    int num_steps) {
  // Calculate the time interval and the angle increment for each step.
  double time_interval = total_time / num_steps;
  double angle_increment = t_a / num_steps;
  ...
  for (int i = 0; i <= num_steps; ++i) {
    ...
    double current_angle = i_a + i * angle_increment;
    // Calculate the pose.
    Eigen::Vector3d pose(std::cos(current_angle), std::sin(current_angle), 0);
...
int DoMain() {
  ...
  const char* kModelUrl =
      "/home/jake/drake/examples/jake/jakes_pendulum.sdf";
...
//   auto iiwa_instance =
  ...
  double travel_angle(3); //radians
  double initial_angle(0); //radians
  ...
  auto [angular_trajectory, times] = MakeAngularTrajectory(initial_angle, travel_angle, end_time, num_steps);
  ...
  // Loop over each time step to solve the IbazK problem.
  for (double t : times){
  ...
  //   double t = segment_start_time;
    ...
    Eigen::VectorXd desired_position = angular_trajectory.value(t);
    std::cout << "q_nominal: [";
    ...
    while (attempt < max_retries && !satisfactory) {
      ...
      if (result.is_success()) {
          ...
          if (n_0 == 0){
            ...
            q_nominal << 2 * M_PI * n_0 + joint_angles[0];
            ik.get_mutable_prog()->SetInitialGuess(ik.q(), q_nominal);
            // std::cout << "false\n";
          }
      }
  ...
  for (int i = 0; i < num_waypoints; ++i) {
    ...
    for (int row = 0; row < joint_angles_trajectory[i].rows(); ++row) {
      for (int col = 0; col < joint_angles_trajectory[i].cols(); ++col) {
        std::cout << joint_angles_trajectory[i](row, col) << " ";
      }
  ...
  // Access the underlying PiecewisePolynomial
  int num_waypoints_foh = joint_angles_trajectory_poly.get_number_of_segments();  // Get the number of segments

  // Loop through each segment (or waypoint) in the PiecewisePolynomial
  for (int i = 0; i < num_waypoints_foh; ++i) {
    std::cout << "FOH Waypoint " << i + 1 << ":\n";

    // Assuming you're printing the polynomial for each segment
    const auto& current_polynomial_matrix_foh = joint_angles_trajectory_poly.getPolynomialMatrix(i);

    // Iterate through the rows and columns of the polynomial matrix for this segment
    for (int row = 0; row < current_polynomial_matrix_foh.rows(); ++row) {
      for (int col = 0; col < current_polynomial_matrix_foh.cols(); ++col) {
          std::cout << current_polynomial_matrix_foh(row, col) << " ";
      }
      std::cout << '\n';  // Newline after each row
    }
    std::cout << '\n';  // Extra newline for separation between waypoints
  }
  ...
  joint_velocity_lower_limit << -0.1;
  joint_velocity_upper_limit << 0.1;
  ...
  joint_torque_lower_limit << -5;
  joint_torque_upper_limit << 5;
  ...
  if (new_joint_angles_poly.has_value()) {
    // Access the underlying PiecewisePolynomial
    const auto& piecewise_poly = new_joint_angles_poly.value();  // Or *new_joint_angles_poly
    int num_waypoints_toppra = piecewise_poly.get_number_of_segments();  // Get the number of segments

    // Loop through each segment (or waypoint) in the PiecewisePolynomial
    for (int i = 0; i < num_waypoints_toppra; ++i) {
      std::cout << "Toppra Waypoint " << i + 1 << ":\n";

      double segment_start_time = piecewise_poly.start_time(i);
      double segment_end_time = piecewise_poly.end_time(i);

      // Print the start and end times for the current segment
      std::cout << "Toppra Segment " << i + 1 << " Time Interval: [" 
                << segment_start_time << ", " << segment_end_time << "]\n";

      // Assuming you're printing the polynomial for each segment
      const auto& current_polynomial_matrix = piecewise_poly.getPolynomialMatrix(i);

      // Iterate through the rows and columns of the polynomial matrix for this segment
      for (int row = 0; row < current_polynomial_matrix.rows(); ++row) {
        for (int col = 0; col < current_polynomial_matrix.cols(); ++col) {
            std::cout << current_polynomial_matrix(row, col) << " ";
        }
        std::cout << '\n';  // Newline after each row
      }
      std::cout << '\n';  // Extra newline for separation between waypoints
      ...
      std::cout << "No joint angles polynomial available.\n";
  }
